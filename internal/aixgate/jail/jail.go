// Package jail provides the platform-agnostic sandbox boundary for aixgate.
//
// v0.1 scope: a single hardcoded policy that hides .env files, SSH private
// keys, and AWS credentials from the sandboxed process. The Jailer interface
...
// Jailer launches a command inside a deny-by-default sandbox configured by
...
// *exec.ExitError on a non-zero exit. Errors related to setting up the jail
// itself (writing the SBPL profile, creating the FUSE mount, etc.) are
// returned before the child is spawned and are wrapped with %w so callers
// can use errors.Is and errors.As.
type Jailer interface {
	Run(ctx context.Context, cmd string, args []string) error
}

// New returns the platform-specific Jailer for the current OS, configured
